...
from .modules import authorize, cancel_mirror, mirror_leech, status, ytdlp, shell, executor, users_settings, torrent_select, bot_settings, speedtest, images, mediainfo, broadcast
...
async def restart(client, message):
    ...
    proc1 = await create_subprocess_exec('pkill', '-9', '-f', '-e', 'gunicorn|aria2c|xnox|xtra|xone')
...
async def main():
    await gather(start_cleanup(), restart_notification(), set_commands(bot))
